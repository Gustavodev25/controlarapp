...
function toIsoFromMillis(value) {
    const millis = parseAppleMillis(value);
    return millis ? new Date(millis).toISOString() : null;
}

function buildAppleRepairPreview({
    source,
    status,
    hasPro,
    periodStartMs,
    ...
    cancelAtPeriodEnd,
    storeEnvironment,
    usedFallbackExpiration,
    serverStatusCode,
    gracePeriodFallbackApplied,
}) {
    const expiresAt = toIsoFromMillis(expiresMs);
    ...
    return {
        ...
        periodStartAt: toIsoFromMillis(periodStartMs || startedMs),
        startedAt: toIsoFromMillis(startedMs || periodStartMs),
        expiresAt,
        ...
        serverStatusCode: serverStatusCode == null ? null : serverStatusCode,
        appleExpirationFallbackApplied: usedFallbackExpiration === true,
        appleGracePeriodFallbackApplied: gracePeriodFallbackApplied === true,
        expiresMs: parseAppleMillis(expiresMs),
    };
}

function buildReceiptRepairPreview({ result, latestReceipt, renewalInfo, requestBody = {}, nowMs = Date.now() }) {
    ...
    const hasPro = !!expiresMs && expiresMs > nowMs && !cancellationMs;
    ...
    return buildAppleRepairPreview({
        source: 'receipt',
        status,
        hasPro,
        periodStartMs: entitlementPeriod.periodStartMs,
        startedMs,
        expiresMs,
        transactionId,
        originalTransactionId,
        ...
        storeEnvironment: result.environment || null,
        usedFallbackExpiration: entitlementPeriod.usedFallbackExpiration,
    });
}

function buildStoreKitRepairPreview({ transactionPayload, purchase = {}, nowMs = Date.now() }) {
    ...
    return buildAppleRepairPreview({
        ...
        autoRenewStatus,
        cancelAtPeriodEnd: hasPro && autoRenewStatus === 'disabled',
        storeEnvironment: transactionPayload.environment || purchase?.environmentIOS || null,
        usedFallbackExpiration: entitlementPeriod.usedFallbackExpiration,
    });
}

function buildAppStoreServerRepairPreview({
    transactionPayload,
    renewalPayload,
    statusCode,
    serverEnvironment,
    ...
    const gracePeriodFallbackApplied = hasPro && !gracePeriodExpiresMs && explicitExpiresMs && explicitExpiresMs <= nowMs;
    if (hasPro && (!expiresMs || expiresMs <= nowMs)) {
        expiresMs = nowMs + DAY_MS;
    }
    ...
    return buildAppleRepairPreview({
        ...
        autoRenewStatus,
        cancelAtPeriodEnd: hasPro && autoRenewStatus === 'disabled',
        storeEnvironment: transactionPayload.environment || serverEnvironment || null,
        usedFallbackExpiration: entitlementPeriod.usedFallbackExpiration,
...
function assertCanRepairAppleSubscription({ currentSubscription, allowProviderChange = false }) {
    if (!currentSubscription || allowProviderChange) return;

    const plan = String(currentSubscription.plan || '').trim().toLowerCase();
    const provider = resolveProvider(currentSubscription);
    const status = normalizeSubscriptionStatus(currentSubscription.status);
    const isPaidPlan = plan === 'pro' || plan === 'premium';
    const isActivePaidPlan = isPaidPlan && ACTIVE_SUBSCRIPTION_STATUSES.has(status);

    if (isActivePaidPlan && provider && !APPLE_PROVIDER_VALUES.has(provider)) {
        throw createHttpError(
            409,
            `Refusing to overwrite active ${provider} subscription without allowProviderChange=true`
        );
    }
}

function getRepairSubscriptionCandidates(userData) {
    return [
        userData?.subscription,
        userData?.profile?.subscription,
    ].filter((subscription) => subscription && typeof subscription === 'object');
}

function scoreRepairSubscriptionCandidate(subscription) {
    ...
    if (APPLE_PROVIDER_VALUES.has(provider)) score += 8;
    if (getAppleOriginalTransactionId(subscription)) score += 6;
    if ((plan === 'pro' || plan === 'premium') && ACTIVE_SUBSCRIPTION_STATUSES.has(status)) score += 4;
    if (subscription.expiresAt || subscription.renewalDate || subscription.nextBillingDate) score += 2;
    if (plan) score += 1;
    if (status) score += 1;
...
function resolveRepairCurrentSubscription(userData) {
    return getRepairSubscriptionCandidates(userData)
        .sort((a, b) => scoreRepairSubscriptionCandidate(b) - scoreRepairSubscriptionCandidate(a))[0] || null;
}

async function validateReceiptForRepair(receiptData) {
    if (!process.env.APPLE_SHARED_SECRET) {
        throw createHttpError(503, 'APPLE_SHARED_SECRET is not configured');
    }
    ...
    if (!VALID_APPLE_RECEIPT_STATUSES.has(result.status)) {
        throw createHttpError(422, `Apple validation failed (status ${result.status})`);
    }
    ...
    if (!latestReceipt) {
        throw createHttpError(404, `No ${PRO_PRODUCT_ID} transaction found in receipt`);
    }

    return {
        result,
        latestReceipt,
        renewalInfo: getRenewalInfo(result, latestReceipt),
    };
}

async function resolveAppleRepairSource({
    currentSubscription,
    receiptData,
    ...
    const normalizedPurchase = purchase && typeof purchase === 'object' ? purchase : {};

    if (signedTransactionInfo) {
        const transactionPayload = decodeAppleTransactionJws(signedTransactionInfo);
        const productId = transactionPayload.productId || normalizedPurchase.productId;
        const bundleId = transactionPayload.bundleId;

        if (bundleId !== APP_BUNDLE_ID) {
            throw createHttpError(400, 'Apple transaction bundle mismatch');
        }

        if (productId !== PRO_PRODUCT_ID) {
            throw createHttpError(400, 'Apple transaction product mismatch');
        }

        return {
            preview: buildStoreKitRepairPreview({ transactionPayload, purchase: normalizedPurchase, nowMs }),
            persistInput: { source: 'storekit', transactionPayload, purchase: normalizedPurchase },
        };
    }

    if (receiptData) {
        const receipt = await validateReceiptForRepair(receiptData);

        return {
            preview: buildReceiptRepairPreview({
                result: receipt.result,
                latestReceipt: receipt.latestReceipt,
                renewalInfo: receipt.renewalInfo,
                requestBody: { transactionId, originalTransactionId },
                nowMs,
            }),
            persistInput: {
                source: 'receipt',
    ...
    const lookupTransactionId = String(
        transactionId ||
        originalTransactionId ||
        getAppleOriginalTransactionId(currentSubscription) ||
        ''
    ).trim();
    ...
    if (!lookupTransactionId) {
        throw createHttpError(
            400,
            'Provide receiptData, signedTransactionInfo, transactionId, or originalTransactionId'
        );
    }

    const preferred = preferredEnvironment || currentSubscription?.storeEnvironment || currentSubscription?.environment || '';
    const { responseBody, environment } = await fetchAppleServerSubscriptionStatuses(lookupTransactionId, preferred);
    const latestTransaction = getLatestProServerApiTransaction(responseBody);

    if (!latestTransaction) {
        throw createHttpError(404, `No ${PRO_PRODUCT_ID} subscription found in App Store Server API response`);
    }

    return {
        preview: buildAppStoreServerRepairPreview({
...
async function persistResolvedAppleRepair({ firebaseUid, resolved }) {
    if (resolved.persistInput.source === 'storekit') {
        return persistStoreKitSubscription({
            firebaseUid,
            transactionPayload: resolved.persistInput.transactionPayload,
            purchase: resolved.persistInput.purchase,
        });
    }

    if (resolved.persistInput.source === 'receipt') {
        return persistAppleSubscription({
            firebaseUid,
            result: resolved.persistInput.result,
            latestReceipt: resolved.persistInput.latestReceipt,
            renewalInfo: resolved.persistInput.renewalInfo,
            requestBody: resolved.persistInput.requestBody,
        });
    }
    ...
    return persistAppStoreServerSubscription({
        firebaseUid,
        transactionPayload: resolved.persistInput.transactionPayload,
        renewalPayload: resolved.persistInput.renewalPayload,
        statusCode: resolved.persistInput.statusCode,
        serverEnvironment: resolved.persistInput.serverEnvironment,
    });
}

async function persistAppleRepairAudit({ admin, userRef, preview, reason, requestId, caseDate }) {
    const serverTimestamp = admin.firestore.FieldValue.serverTimestamp();
    const update = {};
    const repairSummary = {
        source: preview.source,
        reason: reason || 'ios_apple_subscription_repair',
        caseDate: caseDate || null,
        requestId: requestId || null,
        transactionId: preview.transactionId || null,
        originalTransactionId: preview.originalTransactionId || null,
        status: preview.status,
        hasPro: preview.hasPro,
        expiresAt: preview.expiresAt || null,
        appliedAt: serverTimestamp,
    };

    mirrorSubscriptionField(update, 'appleRepairLastAppliedAt', serverTimestamp);
    mirrorSubscriptionField(update, 'appleRepairLastReason', repairSummary.reason);
    mirrorSubscriptionField(update, 'appleRepairLastSource', preview.source);
    mirrorSubscriptionField(update, 'appleRepairLastCaseDate', repairSummary.caseDate);
    mirrorSubscriptionField(update, 'appleRepairLastRequestId', repairSummary.requestId);

    await userRef.set(update, { merge: true });

    const auditId = `apple_repair_${preview.transactionId || preview.originalTransactionId || Date.now()}`;
    await userRef.collection('adminEvents').doc(auditId).set({
        type: 'apple_subscription_repair',
        ...repairSummary,
    }, { merge: true });
}

async function repairAppleSubscriptionForUid({
    firebaseUid,
    receiptData,
    signedTransactionInfo,
    purchase = {},
    ...
    if (!uid) {
        throw createHttpError(400, 'Missing firebaseUid');
    }
    ...
    if (!userDoc.exists) {
        throw createHttpError(404, `User ${uid} not found`);
    }

    const userData = userDoc.data() || {};
    const subscriptionCandidates = getRepairSubscriptionCandidates(userData);
    const currentSubscription = resolveRepairCurrentSubscription(userData);

    for (const subscription of subscriptionCandidates) {
        assertCanRepairAppleSubscription({ currentSubscription: subscription, allowProviderChange });
    }

    const resolved = await resolveAppleRepairSource({
        currentSubscription,
        receiptData,
        signedTransactionInfo,
        purchase,
    ...
    let persisted = null;
    if (apply === true) {
        persisted = await persistResolvedAppleRepair({ firebaseUid: uid, resolved });
        await persistAppleRepairAudit({
            admin,
            userRef,
    ...
    return {
        ...
        applied: apply === true,
        dryRun: apply !== true,
        before: buildStatusSnapshot(currentSubscription),
        repair: resolved.preview,
        result: persisted ? {
            ...
            autoRenewStatus: persisted.autoRenewStatus || null,
...
module.exports._test = {
    ...
    buildAppStoreServerRepairPreview,
