...
const injectAndEnforceUser = (req, res, next) => {
    ...
    if (requestUserId && requestUserId !== req.user.uid) {
        console.error(`[SECURITY] IDOR block. Token: ${req.user.uid} | IP: ${req.ip}`);
...
router.use((req, res, next) => {
    if (req.path === '/oauth-callback' || req.path === '/ping') return next();
...
router.post('/create-item', validate(createItemSchema), async (req, res) => {
    try {
        ...
        if (!response.ok) {
            ...
            if (data.codeDescription === 'ITEM_IS_ALREADY_UPDATING') errorMessage = 'Conexão já em andamento. Aguarde.';
            else if (data.message || data.error) errorMessage = data.message || data.error;

            return res.status(response.status).json({
                success: false, error: errorMessage, details: { code: data.code, description: data.codeDescription }
            });
        }
